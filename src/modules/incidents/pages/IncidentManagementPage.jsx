...
const SEVERITY_MAP = {
  critical: { label: 'Crítico', color: '#ef4444', bg: 'rgba(239, 68, 68, 0.15)', border: 'rgba(239, 68, 68, 0.4)' },
  high: { label: 'Alto', color: '#f97316', bg: 'rgba(249, 115, 22, 0.15)', border: 'rgba(249, 115, 22, 0.4)' },
  medium: { label: 'Medio', color: '#eab308', bg: 'rgba(234, 179, 8, 0.15)', border: 'rgba(234, 179, 8, 0.4)' },
  low: { label: 'Bajo', color: '#3b82f6', bg: 'rgba(59, 130, 246, 0.15)', border: 'rgba(59, 130, 246, 0.4)' }
}
...
const STATUS_MAP = {
  open: { label: 'Abierto', color: '#3b82f6', bg: 'rgba(59, 130, 246, 0.15)', border: 'rgba(59, 130, 246, 0.3)' },
  investigating: { label: 'En Investigación', color: '#eab308', bg: 'rgba(234, 179, 8, 0.15)', border: 'rgba(234, 179, 8, 0.3)' },
  resolved: { label: 'Resuelto', color: '#22c55e', bg: 'rgba(34, 197, 94, 0.15)', border: 'rgba(34, 197, 94, 0.3)' },
  closed: { label: 'Cerrado', color: '#6b7280', bg: 'rgba(107, 114, 128, 0.15)', border: 'rgba(107, 114, 128, 0.3)' }
}
...
export default function IncidentManagementPage() {
  ...
  const countInvestigating = incidents.filter(i => i.status === 'investigating').length
  const countResolved = incidents.filter(i => i.status === 'resolved' || i.status === 'closed').length
  const countAll = incidents.length

  const getSeverityConfig = (sev) => SEVERITY_MAP[sev] || SEVERITY_MAP.medium
  const getTypeConfig = (t) => TYPE_MAP[t] || { label: t || 'Incidente', icon: '⚠️' }
  const getStatusConfig = (s) => STATUS_MAP[s] || { label: s || 'Desconocido', color: '#999', bg: 'rgba(255,255,255,0.1)' }
      ...
      <div className="inc-mgmt__content">
        ...
        <div className="inc-mgmt__sidebar">
          <div className="inc-mgmt__filters">
            ...
            >
              En Investigación <span className="inc-mgmt__tab-count">{countInvestigating}</span>
            </button>
            <button 
              className={`inc-mgmt__filter-btn ${filter === 'resolved' ? 'inc-mgmt__filter-btn--active' : ''}`}
              onClick={() => setFilter('resolved')}
            >
              Resueltos <span className="inc-mgmt__tab-count">{countResolved}</span>
            </button>
            <button 
          ...
          <div className="inc-mgmt__list">
            {filteredList.length === 0 ? (
              <div className="inc-mgmt__empty">No hay incidentes en esta sección</div>
            ) : (
              filteredList.map(inc => {
                ...
                return (
                  ...
                  >
                    <div className="inc-mgmt__card-severity" style={{ background: sevConfig.color }} />
                    <div className="inc-mgmt__card-info">
                      ...
                      <div className="inc-mgmt__card-meta">
                        ...
                        />
                        <span className="inc-mgmt__card-status-text">{statusConfig.label}</span>
                      </div>
                      {inc.geofenceName && (
                    ...
                    {thumbnails.length > 0 && (
                      <div className="inc-mgmt__card-thumb">
                        {imgErrors[thumbnails[0].id] ? (
                          <div className="inc-mgmt__card-thumb-error" title="Error de permisos Appwrite">⚠️</div>
                        ) : (
                          <img 
                            src={thumbnails[0].url} 
                            alt="Evidencia" 
                            loading="lazy" 
                            onError={() => setImgErrors(prev => ({ ...prev, [thumbnails[0].id]: true }))}
                          />
                        )}
                      </div>
                    )}
            ...
            <>
              <div className="inc-mgmt__detail-header">
                <div>
                  <h2 className="inc-mgmt__detail-title">
                    {getTypeConfig(activeIncident.type).icon} {getDisplayTitle(activeIncident)}
                  </h2>
                  <div className="inc-mgmt__detail-badges">
                    <span 
                      className="inc-mgmt__badge" 
                      style={{ 
                        background: getSeverityConfig(activeIncident.severity).bg, 
                        color: getSeverityConfig(activeIncident.severity).color,
                        border: `1px solid ${getSeverityConfig(activeIncident.severity).border}`
                      }}
                    >
                      Prioridad: {getSeverityConfig(activeIncident.severity).label}
                    </span>
                    <span 
                      className="inc-mgmt__badge" 
                      style={{ 
              ...
              <div className="inc-mgmt__reporter">
                ...
                <div className="inc-mgmt__reporter-location">
                  📍 {activeIncident.geofenceName || 'Ubicación General'}{activeIncident.routeName ? ` · ${activeIncident.routeName}` : ''}
                </div>
              </div>
              ...
              {activeIncident.evidenceIds && activeIncident.evidenceIds.length > 0 && (
                <div className="inc-mgmt__evidence">
                  <div className="inc-mgmt__evidence-title">📎 Evidencia Multimedia ({activeIncident.evidenceIds.length})</div>
                  <div className="inc-mgmt__evidence-grid">
                    {getEvidenceThumbnails(activeIncident.evidenceIds).map(thumb => (
                      <div 
                        ...
                        onClick={() => !imgErrors[thumb.id] && openMediaModal(thumb.url)}
                      >
                        {imgErrors[thumb.id] ? (
                          <div className="inc-mgmt__media-error">
                            <span className="inc-mgmt__error-icon">⚠️</span>
                            <span className="inc-mgmt__error-text">Permisos Appwrite</span>
                            <span className="inc-mgmt__error-subtext">Verifique lectura pública en consola</span>
                          </div>
                        ) : (
                          <img 
                            src={thumb.url} 
                            alt="Evidencia" 
                            loading="lazy" 
                            onError={() => setImgErrors(prev => ({ ...prev, [thumb.id]: true }))}
                          />
                        )}
                      </div>
                    ))}
                ...
                <div className="inc-mgmt__actions">
                  ...
                  {activeIncident.status === 'investigating' && (
                    <button 
                      ...
                      onClick={() => handleStatusUpdate(activeIncident.id, 'open')}
                    >
        ...
        <div className="inc-mgmt__media-overlay" onClick={closeMediaModal}>
          <div className="inc-mgmt__media-modal" onClick={(e) => e.stopPropagation()}>
            <div className="inc-mgmt__media-controls">
              ...
              <button className="inc-mgmt__media-close" onClick={closeMediaModal}>✕ Cerrar</button>
            </div>
